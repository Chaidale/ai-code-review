...
AI 前端代码 Review 工具，monorepo 结构，包含 Express 后端服务和 Vue 3 前端界面。通过 DeepSeek API 对前端代码进行 Code Review，同时支持分析 GitHub Pull Request、将 AI 评论发布到 PR 下，以及结合错误/性能摘要做 PR 风险分析。
...
- **PR + 监控分析**：输入 PR 链接 + 错误摘要/性能摘要 → 调用 `/api/review-pr-monitoring`

开发环境通过 Vite 代理 `/api` 到后端；Docker 生产镜像中由 Nginx 反向代理 `/api` 到 `server:3001`。
